    ...
    for (const label of ['Post', 'โพสต์', 'ลงประกาศ', 'Share', 'แชร์']) {
      ...
      if (btn) {
        const disabled = await page.evaluate(el =>
          el.disabled || el.getAttribute('aria-disabled') === 'true', btn);
        if (!disabled) {
          await btn.click();
  ...
  try {
    const found = await page.evaluate(() => {
      ...
      for (const btn of buttons) {
        ...
        if (!disabled && postWords.includes(text)) {
          btn.click();
          return `exact: "${btn.textContent.trim()}"`;
        }
        if (!disabled && postWords.includes(aria)) {
          btn.click();
          return `aria: "${aria}"`;
        }
      }
      ...
      for (const btn of buttons) {
        const text = btn.textContent.trim().toLowerCase();
        const disabled = btn.disabled || btn.getAttribute('aria-disabled') === 'true';
        if (!disabled && postWords.some(w => text.includes(w))) {
          btn.click();
  ...
  try {
    const allButtons = await page.evaluate(() =>
      [...document.querySelectorAll('button, [role="button"]')]
        .slice(0, 20)
        .map(el => `"${el.textContent.trim().slice(0, 25)}" | aria="${el.getAttribute('aria-label') || ''}" | disabled=${el.disabled || el.getAttribute('aria-disabled') === 'true'}`)
    );
